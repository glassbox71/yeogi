$(function () {
    ...
    // 네비 메뉴 선택
    $(".lo-nav>li>a").on("click", function (e) {
        e.preventDefault(); // a 태그의 기본 동작 방지

        // 모든 li에서 .active 제거
        $(".lo-nav>li>a").removeClass("active");

        // 현재 클릭된 a 태그의 부모 li에 .active 추가
    ...
    $(window).on("scroll", function () {
        ...
        let target = $(".lo-fix-wrap");

        // 스크롤이 900 이상일 때 위치를 고정
        if (posY >= 900 && posY <= 2850) {
            target.css("transform", `translateY(${posY - 900}px)`);
        }
        else {
